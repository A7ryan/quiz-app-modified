...
export const registerUser = async (name, email, password) => {
  try {
    ...
    if (error.response) throw error.response.data?.message || "Registration failed";
    else if (error.request) throw "No response from server. Possible network/CORS issue.";
    else throw error.message || "Unexpected error occurred";
  }
};
